...
#		ta	<<	'<tr><table border="1">' # start new table
		...
		wpcw	=	100.0 * wsuw / wsut
		wpcw	=	0.0	if	wpcw.nan?
		ypcw	=	100.0 * ysuw / ysut
		...
		ta	<<	"<td>#{watsdiv}ATS wins->#{watsw} loses->#{watsl} % win is #{100.0 * watsw / (watst == 0 ? 1 : watst)} profit is #{watsw - 1.1 * watsl}</div></td>"
		ta	<<	"<td>#{woudiv}OU wins->#{wouright} loses->#{woul} % win is #{100.0 * wouright / (wout == 0 ? 1 : wout)} profit is #{wouright - 1.1 * woul}</div></td>"
		ta	<<	"<td>#{wmldiv}Money Line wins->#{wmlw} loses->#{wmll} % win is #{100.0 * wmlw / ((wmlw + wmll) == 0 ? 1 : (wmlw + wmll))} profit is #{wmlp}</div></td>"

		# year
		ta	<<	"<td>#{ysudiv}Year #{week} SU wins->#{ysuw} loses->#{ysul} % win is #{ypcw}</div></td>"
		ta	<<	"<td>#{yatsdiv}ATS wins->#{yatsw} loses->#{yatsl} % win is #{100.0 * yatsw / (yatst == 0 ? 1 : yatst)} profit is #{yatsw - 1.1 * yatsl}</div></td>"
		ta	<<	"<td>#{youdiv}OU wins->#{youright} loses->#{youl} % win is #{100.0 * youright / (yout == 0 ? 1 : yout)} profit is #{youright - 1.1 * youl}</div></td>"
		ta	<<	"<td>#{ymldiv}Money Line wins->#{ymlw} loses->#{ymll} % win is #{100.0 * ymlw / ((ymlw+ymll) == 0 ? 1 : (ymlw+ymll))} profit is #{ymlp}</div></td>"
